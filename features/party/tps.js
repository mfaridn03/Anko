...
import { clientSay, Packets } from "../../utils/consts";
...
Events.addPartyChatListener(
    () => {
        ...
        if (diff < WAIT)
            clientSay(`TPS still loading, ${((WAIT - diff) / 1000).toFixed(1)}s remaining`)
        else
            clientSay(`TPS: ${(packets.size() / (WAIT / 1000)).toFixed(1)}`)
    },
